...
func startCrawl(url string, depth int, visitRegister visitreg.VisitRegister, fetcher Fetcher) map[string]bool {
  ...
  go reccrawl(done, url, depth, visitRegister, fetcher)
  <-done
...
func reccrawl(done chan<- struct{}, url string, depth int, visitRegister visitreg.VisitRegister, fetcher Fetcher) {
  ...
  for _, u := range urls {
    go reccrawl(childrenDone, u, depth-1, visitRegister, fetcher)
  }
  for i := 0; i < len(urls); {
    <-childrenDone
    i++
